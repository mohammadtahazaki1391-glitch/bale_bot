...
def home():
    ...
    for country, flag in COUNTRIES.items():
        taken = country in data["countries"]
        conquered = is_conquered(country) if taken else False
...
def war(country):
    ...
    for c, f in COUNTRIES.items():
        ...
        countries_list.append({
            ...
            "gold": data["countries"][c].get("gold", 0),
...
def attack(attacker, defender):
    ...
    ensure_country(attacker); ensure_country(defender)
...
def admin():
    ...
    <style>
        ...
        .btn { padding:5px 8px; border:none; border-radius:5px; color:white; text-decoration:none; font-size:11px; }
            ...
            <table>
                ...
                {% for country, flag in countries.items() %}
                    ...
                    <tr>
                        ...
                        <td>{% if status[country].get('conquered_by') %}🔥 فتح{% else %}✅ فعال{% endif %}</td>
                        <td>
